...
function parseMonth(value: string): { start: Date; endExclusive: Date } | null {
...
router.get('/', async (req, res) => {
  ...
  if (req.query.siteId != null && !Number.isFinite(siteId)) return res.status(400).json({ error: 'Invalid siteId' });
  ...
  const docs = jobs
    .map((j) => {
      ...
      const reportCreatedAt =
        j.cleaningJob?.reportCreatedAt ?? j.serviceJob?.reportCreatedAt ?? j.inspectionJob?.reportCreatedAt ?? null;
