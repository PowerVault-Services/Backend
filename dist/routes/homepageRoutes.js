...
exports.homepageRoutes = (0, express_1.Router)();
function calcPlantStatus(inverters) {
 ...
 */
exports.homepageRoutes.get('/summary', async (_req, res) => {
    try {
        ...
        for (const inv of inverters) {
            if (!bySite.has(inv.siteId))
                bySite.set(inv.siteId, []);
        ...
        const active = await prisma_1.default.alarm.findMany({
        ...
        const latest = await prisma_1.default.alarm.findMany({
            where: { status: 'ACTIVE', clearedAt: null },
 ...
 *  - pagination
 */
exports.homepageRoutes.get('/plants', async (req, res) => {
    try {
        ...
        for (const inv of invs) {
            if (!invBySite.has(inv.siteId))
                invBySite.set(inv.siteId, []);
            invBySite.get(inv.siteId).push(inv);
        }
        // yield today (จาก SiteDailyEnergy วันนี้)
        const today = new Date();
        today.setHours(0, 0, 0, 0);
        const energyRows = await prisma_1.default.siteDailyEnergy.findMany({
            where: { siteId: { in: siteIds }, date: today },
            select: { siteId: true, energyKWh: true },
        });
        const energyMap = new Map();
        for (const r of energyRows)
            energyMap.set(r.siteId, safeNum(r.energyKWh));
        // total yield
        const latestSnaps = await prisma_1.default.inverterKpiSnapshot.findMany({
        ...
        const latestTotalByInv = new Map();
